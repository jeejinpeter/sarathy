...
public function add_invoice_spare()
{
	...
	$re=$this->Bill_model->checkinvoce($invoic);
	if($re == null){
...
public function list_invoice()
 {
	 ...
	 $data['invo'] = $this->Bill_model->list_invoice_staff($brancid);
	...
	if($bill)
	{
	  ...
	  $data=array(
			...
			$form_data=  $this->input->post();
						...
						$result1=$this->Bill_model->invoice_dtls_ready($form_data,$data);
			if($result1){
...
public function list_custom_date_staff()
  ...
  $data['list'] = $this->Admin_model->list_custom_date_mech($from,$to,$mech,$branch,$advisor,$service);
...
public function list_job_card_statement_proccessing()
  ...
  $data['list'] = $this->Admin_model->list_job_card_statement($from,$to,$mech,$branch,$code,$advisor,$service,$repair);
...
$exist=$this->Bill_model->check_exist_labour($id);	
if($exist==null){
...
$upd_data=array('inv_total' =>  $this->input->post('total'),
					'inv_disc_total' =>  $this->input->post('tdisc'),
                    'inv_gsttotal' =>  $this->input->post('gstotl'),
                    'inv_sgstotal' =>  $this->input->post('sgtotal'),
                    'inv_taxtotal' =>  $this->input->post('ttotl'),
					...
					'inv_inv_date' =>  $this->input->post('invoicedate'),
                    'inv_id' => $id);
$result2=$this->Bill_model->update_inv($upd_data);
$data = array(
				'ready_status'=>0
...
redirect('Admin/pdf_for_ready_bill/'.$id);
	  ...
	  $data=array(
			...
			$form_data=  $this->input->post();
			$result=$this->Bill_model->invoice_dtls_ready_new($form_data,$data,$invid);
			if($result){
...
public function list_insurance_invoice()
 {
	  $brancid= $this->Bill_model->branch_name_select_id();
      $data['invo'] = $this->Bill_model->list_insurance_invoice_staff($brancid);
...
elseif($to2[1]=='12')
{
	...
	//$total=$row->lc_amount;	
 $sub_array[]='<input type="hidden" class="lc_amt" value="'.$total.'"><a style="font-size:11px;" href="'.base_url().'Admin/edit_jobcard/'.$row->inv_id.'">'.$row->inv_job_card_no.'</a>';				
                //$sub_array[] = '<img src="'.base_url().'upload/'.$row->image.'" class="img-thumbnail" width="50" height="35" />';
