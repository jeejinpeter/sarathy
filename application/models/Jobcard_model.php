...
class Jobcard_model extends CI_Model {
	...
	var $column_order = array(null,"a.inv_job_card_no", "a.inv_jcard_date","tbl_branch.branch_name","c.e_first_name AS mech_name","d.e_first_name AS adv_name","a.inv_modl","","a.inv_repair_typ","a.inv_no","a.inv_cus","a.inv_pho","a.inv_inv_date", "a.in_registr", "a.inv_chassis","a.in_engine","a.inv_km","tbl_insurance_company.icompany_name","tbl_insurance_company.insurance_serveyor","b.lc_amount","SUM(b.lc_rate) as id_sum","SUM(b.lc_rate) as id_sum","a.inv_cus_gstin","a.inv_disc_total","a.inv_taxtotal","a.inv_sgstotal","a.inv_gsttotal","a.inv_type","a.inv_total"); //set column field database for datatable orderable
	 
	var $column_search = array("a.inv_id", "a.inv_job_card_no", "a.inv_jcard_date","tbl_branch.branch_name","c.e_first_name AS mech_name","d.e_first_name AS adv_name","a.inv_modl","","a.inv_repair_typ","a.inv_no","a.inv_cus","a.inv_pho","a.inv_inv_date", "a.in_registr", "a.inv_chassis","a.in_engine","a.inv_km","tbl_insurance_company.icompany_name","tbl_insurance_company.insurance_serveyor","b.lc_amount","SUM(b.lc_rate) as id_sum","SUM(b.lc_rate) as id_sum","a.inv_cus_gstin","a.inv_disc_total","a.inv_taxtotal","a.inv_sgstotal","a.inv_gsttotal","a.inv_type","a.inv_total"); //set column field database for datatable searchable 
...
private function _get_datatables_query2()
	{
   $this->db->select('SUM(b.lc_rate) as id_sum');  
    $this->db->from($this->table); 		
	$this->db->join('tbl_invoice_labour_cost b','b.ic_inv_id=a.inv_id','LEFT');
	$this->db->join('tbl_employee c', "c.emp_id = a.inv_advisername",'LEFT');
    $this->db->join('tbl_employee d', "d.emp_id = a.inv_mechna",'LEFT');
...
elseif($to2[1]=='12')
{
    $to=date('Y-m-31', strtotime('-1 month'));
	    ...
	    $this->db->where('a.inv_jcard_date>=',$from);
		$this->db->where('a.inv_jcard_date<=',$to);
		//add custom filter here
		if($this->input->post('branch'))
		{
		$this->db->where('a.inv_branch', $this->input->post('branch'));
