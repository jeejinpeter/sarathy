...
class Bill_model extends CI_Model {
  public function __construct()
   ...
   public function checkinvoce_labour($invoic)
 {
$this->db->select('*');
...
return $query;		
 ...
 public function checjobcrd_labour($jbno)
 {
$this->db->select('*');
$this->db->from('tbl_readyfor_labour');
...
return $query;		
   ...
   public function view_invoice_customer_insurance_staff($id)
 {
$this->db->select('*,e1.e_first_name as advai,e2.e_first_name as mechni,e1.emp_id as advai_id,e2.emp_id as mechni_id');
$this->db->from('tbl_readyfor_labour');
$this->db->join('tbl_branch', "tbl_branch.b_id = tbl_readyfor_labour.inv_branch",'left');
$this->db->join('tbl_employee e1', "e1.emp_id = tbl_readyfor_labour.inv_advisername",'left');
$this->db->join('tbl_employee e2', "e2.emp_id = tbl_readyfor_labour.inv_mechna",'left');
$this->db->join('tbl_insurance_company', "tbl_insurance_company.com_id = tbl_readyfor_labour.insurance_id",'left');
$this->db->where('tbl_readyfor_labour.status',1);
$this->db->where('inv_id',$id);
$query = $this->db->get()->result();
return $query;	 
 }
   public function list_insurance_invoice_staff_ready($branch_id)
{
$this->db->select('*');
$this->db->from('tbl_readyfor_labour');
$this->db->join('tbl_branch', "tbl_branch.b_id = tbl_readyfor_labour.inv_branch",'left');
$this->db->where('tbl_readyfor_labour.ready_status',1);
$this->db->where('tbl_readyfor_labour.status',1);
$this->db->where('tbl_readyfor_labour.inv_branch',$branch_id);
$query = $this->db->get()->result();
return $query;		
}
  public function list_invoice_staff_labour($brancid)
{
$this->db->select('*');
$this->db->from('tbl_readyfor_labour');
$this->db->join('tbl_branch', "tbl_branch.b_id = tbl_readyfor_labour.inv_branch",'left');
$this->db->where('tbl_readyfor_labour.inv_branch',$brancid);
$this->db->where('tbl_readyfor_labour.status',0);
$this->db->where('tbl_readyfor_labour.ready_status',1);
$query = $this->db->get()->result();
return $query;		
} 
    public function update_ready_customer_ready($data,$id)
	{
	$this->db->where('inv_id',$id);
	return $query = $this->db->update('tbl_readyfor_labour',$data);
	}
    public function insert_inv_ready($upd_data)
  {
	 $query = $this->db->insert('tbl_invoice_labour',$upd_data);
	  $s_id = $this->db->insert_id();
     return  $s_id;
  }
   
 public function list_insurance_invoice_labour(){
$this->db->select('*');
$this->db->from('tbl_readyfor_labour');
$this->db->join('tbl_branch', "tbl_branch.b_id = tbl_readyfor_labour.inv_branch",'left');
$this->db->where('tbl_readyfor_labour.ready_status',1);
...
return $query;		
  ...
  public function check_exist_labour_ready($id){
$this->db->select('*');
$this->db->from('tbl_readyfor_labour');
$this->db->where('tbl_readyfor_labour.inv_id',$id);
$this->db->where('tbl_readyfor_labour.ready_status',0);
$query = $this->db->get()->result();
return $query;		
 } 
  public function invoice_dtls_ready_new_labour($form_data,$data,$invid)
 {
		$this->db->where('inv_id',$invid);
	    $res = $this->db->update('tbl_readyfor_labour',$data);	
		if($res){
			$last_id = $invid;
 ...
 public function view_invoice_customer_insurance_ready($id)
 {
$this->db->select('*,e1.e_first_name as advai,e2.e_first_name as mechni,e1.emp_id as advai_id,e2.emp_id as mechni_id');
$this->db->from('tbl_readyfor_labour');
$this->db->join('tbl_branch', "tbl_branch.b_id = tbl_readyfor_labour.inv_branch",'left');
...
return $query;	 
    ...
    public function view_invoice_customer_ready($id)
 {
$this->db->select('*,b1.*,e1.e_first_name as advai,e2.e_first_name as mechni,e1.emp_id as advai_id,e2.emp_id as mechni_id');
$this->db->from('tbl_readyfor_labour');
$this->db->join('tbl_branch b1', "b1.b_id = tbl_readyfor_labour.inv_branch",'left');
$this->db->join('tbl_employee e1', "e1.emp_id = tbl_readyfor_labour.inv_advisername",'left');
$this->db->join('tbl_employee e2', "e2.emp_id = tbl_readyfor_labour.inv_mechna",'left');
$this->db->where('inv_id',$id);
$query = $this->db->get()->result();
return $query;	 
   ...
   public function list_invoice_ready_labour(){
$this->db->select('*');
$this->db->from('tbl_readyfor_labour');
$this->db->join('tbl_branch', "tbl_branch.b_id = tbl_readyfor_labour.inv_branch",'left');
$this->db->where('tbl_readyfor_labour.ready_status',1);
...
return $query;		
    ...
    public function invoice_dtls_ready_labour($form_data,$data)
 {
	 	$res=$this->db->insert('tbl_readyfor_labour', $data);	
		if($res){
...
return $query;		
   ...
   public function view_service_data($inv_id) 
 {
$this->db->select('*');
$this->db->from('tbl_invoice_labour_cost');
$this->db->where('ic_inv_id',$inv_id);
$query = $this->db->get()->result();
return $query;	 
 } 
 
   public function view_customer_data($id)
 {
$this->db->select('*');
$this->db->from('customer_details');
$this->db->where('c_reg_no',$id);
$query = $this->db->get()->result();
return $query;	 
 } 
   
  public function view_vehicle_history($id)
 {
$this->db->select('*,b1.*,e1.e_first_name as advai,e2.e_first_name as mechni,e1.emp_id as advai_id,e2.emp_id as mechni_id');
$this->db->from('tbl_invoice_labour');
$this->db->join('tbl_branch b1', "b1.b_id = tbl_invoice_labour.inv_branch",'left');
$this->db->join('tbl_employee e1', "e1.emp_id = tbl_invoice_labour.inv_advisername",'left');
$this->db->join('tbl_employee e2', "e2.emp_id = tbl_invoice_labour.inv_mechna",'left');
$this->db->where('in_registr',$id);
$query = $this->db->get()->result();
return $query;	 
 } 
 public function select_company_by_brnch($branch_id)
	{
	$this->db->select('*');
